...
export const Body: React.FC<Props> = React.memo(
  ({
    ...
    useEffect(() => {
      if (!engine) return;
      const rawBody = Bodies.rectangle(initialX, initialY, initialWidth, initialHeight, {
    ...
    useEffect(() => {
      ...
      const unprocessComposite = (processableBodyGroup: BodyGroup) =>
        processableBodyGroup.delete(body);

      Array.isArray(bodyGroup) ? bodyGroup.map(processComposite) : processComposite(bodyGroup);

      return () => {
        Array.isArray(bodyGroup)
          ? bodyGroup.map(unprocessComposite)
          : unprocessComposite(bodyGroup);
      };
    }, [body, bodyGroup]);
    ...
    useEffect(() => {
      ...
      const cb = (e: Matter.IEventCollision<Engine>) => {
        ...
        const cleanEvent = {
          ...e,
          pairs: e.pairs.filter((pair) => pair.bodyA === body || pair.bodyB === body),
        };
    ...
    useTick((delt) => {
      ...
      if (
        ...
        rotation !== +body.angle.toFixed(3) ||
        vx !== +body.velocity.x.toFixed(3) ||
        vy !== +body.velocity.y.toFixed(3)
      ) {
        setX(+body.position.x.toFixed(3));
        setY(+body.position.y.toFixed(3));
    ...
    const subscribeOnCollision = useCallback((cb: CleanEventListener) => {
      collisionListentersRef.current.push(cb);
    }, []);
    const clearCollisionSubscription = useCallback((cb: CleanEventListener) => {
      collisionListentersRef.current = collisionListentersRef.current.filter(
    ...
    return (
      <BodyStateContextProvider value={value}>
        <BodyContextProvider value={bodyValue}>
          {children}
          {isCollisionVisible ? (
            <RectangleController width={initialWidth} height={initialHeight} />
          ) : null}
        </BodyContextProvider>
      </BodyStateContextProvider>
    );
  },
  ({ options, ...prevProps }, { options: newOptions, ...nextProps }) =>
    isEqual(newOptions, options) && isEqual(prevProps, nextProps),
);
