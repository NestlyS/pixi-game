...
export const SimpleAIController = ({ children, spritesheetUrl, animationName }: Props) => {
  ...
  useEffect(() => {
    ...
    const cb: CleanEventListener = (e) => {
      const isSensorCollision = e.pairs.some(
        (value) => value.bodyA.label === 'ai-sensor' || value.bodyB.label === 'ai-sensor',
      );
  ...
  const handleCollision = useCallback((e: IEventCollision<Engine>) => {
    if (
      isAttackRef.current ||
