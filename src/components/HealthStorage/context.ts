...
export const useHealth = (id?: IdType | null) => {
  ...
  const setCurrentHealth = useCallback((value: ValueType | FuncValueType, _id?: IdType, cooldownTimeout?: number) => {
  ...
  const onCurrentHealthCooldown = useCallback((cb: (cooldown: boolean) => void, _id?: IdType) => {
    const innerId = id || _id;
  ...
  const clearCurrentHealthCooldown = useCallback((cb: (cooldown: boolean) => void, _id?: IdType) => {
    const innerId = id || _id;
    console.log(_id, id, cb);
