...
export const HealthStorage = memo(({ children }: Props) => {
  const [healthState, updateHealthState] = useState(initialState.healthMap);
  const cooldownStateRef = useRef<CooldownState>({});

  const setHealth = useCallback<HealthSetter>(
    (id, value, cooldownTimeout = 0) =>
      updateHealthState((state) => {
        if (!cooldownStateRef.current[id])
          initCooldown(cooldownStateRef.current, id, cooldownTimeout);
        if (cooldownTimeout && cooldownStateRef.current[id].cooldownTimeout !== cooldownTimeout)
          cooldownStateRef.current[id].cooldownTimeout = cooldownTimeout;
        if (state[id] === undefined && typeof value === 'function')
          throw new Error('Health is not settled. Nothing to throw to function');
        ...
        if (
          ...
          cooldownStateRef.current[id].listeners.forEach((cb) =>
            cb(cooldownStateRef.current[id].isCooldown),
          );
          setTimeout(() => {
  ...
  return <HealthContextProvider value={value}>{children}</HealthContextProvider>;
});
