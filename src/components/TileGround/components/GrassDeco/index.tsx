...
import { GRASS_DECORATION_1, GRASS_DECORATION_2, GRASS_DECORATION_3, GRASS_DECORATION_BUSH, GRASS_DECORATION_BUSH_BERRY, GRASS_DECORATION_BUSH_SMALL, GRASS_DECORATION_TREE } from "./constants"
...
(new Array(width / step)).fill(0).reduce((acc: ReducerAccType, _, index, arr) => {
  ...
  }, { percentages: layer.percentages, sprites: [], lastAdditionAgo: 0} as ReducerAccType).sprites;


const getTexuresInLayers = (width: number, y: number, step: number = 5): SpriteParams[][] => Layers.map(item => getLayer(item, width, step, y));
...
export const GrassDeco = memo(({
  ...
  return (
    <>
      {
        textures.map((texArr, index) => texArr
          .map(tex =>
              <Sprite x={x + tex.x} y={tex.y} textureUrl={tex.textureUrl} spritesheet={spritesheetUrl} key={tex.key} anchor={anchor} scale={scale} />
            )
          )
      }
    </>
