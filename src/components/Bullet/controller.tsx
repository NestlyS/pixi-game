...
const getDirectionMultiplier = (direction: Directions): { x: number, y: number } => {
  switch (direction) {
    ...
    case Directions.UpRight: return { x: 1, y: -1 };
    case Directions.Right: return { x: 1, y: 0 };
    case Directions.RightDown: return { x: 1, y: 1};
    case Directions.Down: return { x: 0, y: 1 };
    case Directions.DownLeft: return { x: -1, y: 1 };
    case Directions.Left: return { x: -1, y: 0 };
    case Directions.LeftUp: return { x: -1, y: -1 };
  }
}
...
export const BulletController = ({
  ...
  useEffect(() => {
    ...
    return () => { distanceRef.current = { x: 0, y: 0 }; }
  }, [body]);
  ...
  useTick(delta => {
    ...
    Body.setPosition(body, { x: body.position.x + speed * xMult, y: body.position.y + speed * yMult })
    const distance = Math.sqrt(Math.pow(body.position.x - distanceRef.current.x, 2) - Math.pow(body.position.y - distanceRef.current.y, 2));
