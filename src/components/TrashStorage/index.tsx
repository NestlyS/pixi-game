...
export const TrashStorage = ({
  ...
  const onChange = useCallback<State['onChange']>((id: number | string, type: TrashTypes, cb: (type: TrashTypes, value: number) => void) => {
    if (!trash[id]) trash[id] = initTrash();
    trash[id][type].listeners.push(cb);
  }, [trash]);

  const clearOnChange = useCallback<State['onChange']>((id: number | string, type: TrashTypes, cb: (type: TrashTypes, value: number) => void) => {
  ...
  return (
    <TrashContextProvider value={value}>{children}</TrashContextProvider>
  )
}
