...
const appendChunk = ({
  ...
  return appendChunk({
    chunks: newChunks,
    tileSize,
    chunkWidth: chunkWidth - newChunks[newChunks.length - 1].width,
  });
};
...
export const LevelManager = ({
  ...
  const draw = useCallback(
    (x: number, y: number) => (g: PIXI_Grapics) => {
  ...
  const collisionDetectorX =
    chunks[chunks.findIndex((_, index, arr) => index >= arr.length - 1 - 1)].x;
    ...
    <>
      ...
      {__IS_DEV__ && <Graphics
        draw={draw(
          ...
          chunks[chunks.length - 1].y - tilesHeight * tileSize,
        )}
      />}
      <Body
        width={60}
        height={10000}
        x={collisionDetectorX}
        y={0}
