...
export const TrashBodyController = ({
  amplitude,
  isTouched,
  onDelete,
}: Props) => {
  ...
  useTick(delta => {
    ...
    if (!isTouched && (body.position.y >= (bodyStartPosRef.current.y + amplitude) || body.position.y <= (bodyStartPosRef.current.y - amplitude))) directionRef.current *= -1;
