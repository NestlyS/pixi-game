...
export const Trash = ({
  ...
  const onCollision = useCallback(
    (e: Matter.IEventCollision<Matter.Engine>) => {
      ...
      const collidedPair = e.pairs.find(pair => USER_BODY_GROUP.get().some(body => body.id === pair.bodyA.id || body.id === pair.bodyB.id));
      if (!collidedPair) return;

      const collidedUser = collidedPair.bodyA.label === trashLabelRef.current ? collidedPair.bodyB : collidedPair.bodyA;
  ...
  return (
    <Body x={x} y={y} options={TRASH_PARAMS} width={TRASH_WIDTH} height={TRASH_HEIGHT} onCollision={onCollision} label={trashLabelRef.current}>
      <TrashBodyController amplitude={ANIMATION_AMPLITUDE} isTouched={isTouched} onDelete={onDelete} />
      <AnimatedSpriteController spritesheet={spritesheetUrl} initialAnimation={animationName} width={TRASH_WIDTH} height={TRASH_HEIGHT} zIndex={20} />
    </Body>
  )
}
