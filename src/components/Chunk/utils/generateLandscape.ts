...
export const generateLandscape = (width: number, percents: PercentMapType, blocksBeforeTerrainChange: number = 3): LandscapeOffsetsType =>
  (new Array(width)).fill(0).reduce((acc) => {
    ...
    const addNewLandscapeValue = (value: number = 0) => [...acc, (acc.length ? acc[acc.length - 1] : 0) + value];
