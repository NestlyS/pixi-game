...
const drawMask = (x: number, y: number, width: number, height: number) => (new PIXI_Graphics()).clear().beginFill(0xDE3249, 0.7).drawRect(x, y, width, height).endFill()
  ...
  return (
    <>
      <Button
        ...
        {...buttonProps}
      />
      <Graphics x={x} y={y + height - delta} width={width} height={height} draw={draw} mask={mask} />
    </>
  )
});
